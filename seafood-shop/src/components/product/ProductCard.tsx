      ...
      <div className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden border border-gray-100/50 hover:border-primary/20 card-hover">
        <Link href={`/san-pham/${product.slug}`} className="block">
          ...
          <div className="p-3 md:p-4">
            {/* Product Name */}
            <h3 className="font-semibold text-gray-800 line-clamp-2 min-h-[40px] md:min-h-[48px] text-[13px] md:text-base leading-tight group-hover:text-primary transition-colors duration-300">
              {product.name}
            </h3>

            {/* Price */}
            <div className="mt-2 md:mt-3 flex items-baseline gap-1.5 md:gap-2 flex-wrap">
              <span className="text-base md:text-xl font-bold bg-gradient-to-r from-red-500 to-red-600 bg-clip-text text-transparent">
                {formatPrice(product.price)}
              </span>
              {hasDiscount && product.originalPrice && product.originalPrice > product.price && (
                <span className="text-xs md:text-sm text-gray-400 line-through decoration-gray-300">
            ...
            {/* Unit */}
            {product.unit && (
              <span className="text-[10px] md:text-xs text-gray-500 font-medium">/ {product.unit}</span>
            )}
            ...
            <div className="mt-1.5 md:mt-2 flex items-center justify-between">
              {product.soldCount > 0 ? (
                <p className="text-[10px] md:text-xs text-gray-500 flex items-center gap-1">
                  <svg className="w-3 h-3 md:w-4 md:h-4 text-red-400 drop-shadow-sm" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-2.5 h-2.5 md:w-3 md:h-3 ${i < 4 ? 'text-amber-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        {showAddToCart && product.inStock && (
          <div className="px-3 pb-3 md:px-4 md:pb-4">
            <button
              className="w-full py-2 md:py-2.5 bg-gradient-to-r from-primary to-primary-dark text-white rounded-xl text-xs md:text-sm font-semibold hover:shadow-lg hover:shadow-primary/30 transition-all duration-300 btn-shine active:scale-[0.98]"
